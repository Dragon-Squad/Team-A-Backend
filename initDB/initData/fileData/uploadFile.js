...
const uploadImage = async (filename, folder, destinationFolder) => {
  try {
    const localFilePath = path.join(__dirname, `../../resources/image/${folder}/${filename}`);
...
const uploadVideo = async (filename, uploadFolder) => {
  try {
    const localFilePath = path.join(__dirname, `../../resources/video/${filename}`);

    // Generate a unique file name using the current timestamp
    const timestamp = Date.now();
    const randomStr = Math.random().toString(36).substring(2, 8);
    const fileName = `video-${timestamp}-${randomStr}${path.extname(localFilePath)}`;
