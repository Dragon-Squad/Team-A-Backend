...
class MonthlyDonationService {
    async create() {
        try {
            ...
            throw new Error(
                "Error creating Monthly Donation: " + error.message
            );
        }
    }
    ...
    async getAllMonthlyDonations(page, limit) {
        try {
            ...
            throw new Error(
                "Error fetching all Monthly Donations: " + error.message
            );
        }
    }

    async getAllMonthlyDonationsByDonor(
        page,
        limit,
        status,
        ...
        try {
            ...
            const result = await MonthlyDonationRepository.getAllByDonor(
                page,
                limit,
                donor.userId,
                status,
            ...
            throw new Error(
                "Error fetching Monthly Donations by Donor: " + error.message
            );
        }
    }
    ...
    async getMonthlyDonationsByProject(limit, page, projectId) {
        ...
        try {
            ...
            const monthlyDonations =
                await MonthlyDonationRepository.getAllByProject(
                    limit,
                    page,
                    projectId
                );
            ...
            console.error(
                "Error during monthly donation process:",
                err.message
            );
            throw err;
        } finally {
    ...
    async getMonthlyDonationById(id) {
        try {
            const monthlyDonation = await MonthlyDonationRepository.findById(
                id
            );
            if (!monthlyDonation) {
    ...
    async getByStripeSubscriptionId(stripeSubscriptionId) {
        try {
            const monthlyDonation =
                await MonthlyDonationRepository.getByStripeSubscriptionId(
    ...
    async update(id, data) {
        try {
            const updatedMonthlyDonation =
                await MonthlyDonationRepository.update(id, data);
            if (!updatedMonthlyDonation) {
                throw new Error(
                    "Monthly Donation not found or failed to update"
                );
            }
            return updatedMonthlyDonation;
        } catch (error) {
            throw new Error(
                "Error updating Monthly Donation: " + error.message
            );
        }
    }
    ...
    async cancel(id) {
        try {
            ...
            const cancelledMonthlyDonation =
                await MonthlyDonationRepository.cancel(id);
            if (!cancelledMonthlyDonation) {
                throw new Error(
                    "Monthly Donation not found or failed to cancel"
                );
            }
            return cancelledMonthlyDonation;
        } catch (error) {
            throw new Error(
                "Error cancelling Monthly Donation: " + error.message
            );
        }
    }
}
