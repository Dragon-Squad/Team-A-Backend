...
class ProjectController {
        ...
        try {
            ...
            if (!deleted)
                return res
                    .status(404)
                    .json({
                        message: "Project not found or status isn't halted",
                    });
    ...
    async getById(req, res) {
        try {
            ...
            // Check Redis cache
            const cachedProject = await RedisMiddleware.readData(
                clientId,
                cacheKey
            );
            if (cachedProject) {
    ...
    async getAll(req, res) {
        try {
            ...
            // Check Redis cache
            const cachedProjects = await RedisMiddleware.readData(
                clientId,
                cacheKey
            );
            if (cachedProjects) {
                console.log(
                    "Cache hit for project list with filters:",
                    filters
                );
    ...
    async getTotalProjectStatus(req, res) {
        try {
            const filters = req.query;
            const statusCounts = await ProjectService.getTotalProjectStatus(
                filters
            );
