...
class ProjectExternalService {
    ...
    async updateProjectRaisedAmount(value) {
        try {
            ...
            if (updatedAmount >= project.goalAmount) {
                ...
                const deletionResult = await ProjectRepository.delete(
                    projectId
                );
    ...
    async getNames(value) {
        ...
        const categoryNames =
            await CategoryExternalService.getCategoryNamesByIds(categoryIds);
        const regionNames = await RegionExternalService.getRegionNamesByIds(
            regionIds
        );
